    ...
    for f in folds:
        ...
        for pf in predicted_files:
            ...
            if overwrite or not isfile(join(merged_output_folder, pf)):
                print(f"copy {join(expected_validation_folder, pf)} to {join(merged_output_folder, pf)}")
                shutil.copy(join(expected_validation_folder, pf), join(merged_output_folder, pf))
                did_we_copy_something = True
    print(f"did_we_copy_something {did_we_copy_something}")
    for f in os.listdir(merged_output_folder):
        print(f)
